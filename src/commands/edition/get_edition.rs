            ...
            .interaction_response_data(|message|
                message.embed(|embed| {
                    embed.title(&edition)
                        ...
                        .field("Date de début des inscriptions" , &format!("{}", NaiveDateTime::from_timestamp_opt(inscription_start_date, 0).unwrap().format("%d/%m/%Y")), false)
                        .field("Date de fin des inscriptions"   , &format!("{}", NaiveDateTime::from_timestamp_opt(inscription_end_date  , 0).unwrap().format("%d/%m/%Y")), false)
                        .field("Date de début de la compétition", &format!("{}", NaiveDateTime::from_timestamp_opt(competition_start_date, 0).unwrap().format("%d/%m/%Y")), false)
                        .field("Date de fin de la compétition"  , &format!("{}", NaiveDateTime::from_timestamp_opt(competition_end_date  , 0).unwrap().format("%d/%m/%Y")), false)
                })
            )
    })
        .await
